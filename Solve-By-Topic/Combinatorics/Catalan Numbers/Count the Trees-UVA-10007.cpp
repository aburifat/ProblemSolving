...
void Print(BigInt a)
{
    ...
    frr(i, a.size() - 2, 0) printf("%09d", a[i]);
    nl;
}
...
bool operator<(BigInt a, BigInt b)
{
    ...
    frr(i, a.size() - 1, 0) if (a[i] != b[i]) return (a[i] < b[i]);
    return false;
}
...
BigInt operator-(BigInt a, BigInt b)
{
    ...
    fr(i, 0, a.size() - 1)
    {
        ...
        if (carry < 0)
            ans.pub(carry + base), carry = -1;
        else
            ans.pub(carry), carry = 0;
    }
    Set(ans);
    return ans;
}
